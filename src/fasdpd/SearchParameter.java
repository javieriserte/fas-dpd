...
public class SearchParameter {
	...
	public void retrieveFromCommandLine(String[] args)
			throws InvalidCommandLineException {
		CommandLine cmd = new CommandLine();

		SingleArgumentOption<Integer> lenMin =
			new SingleArgumentOption<Integer>(
				cmd, "/lenMin", new IntegerValue(), 20);
		SingleArgumentOption<Integer> lenMax =
			new SingleArgumentOption<Integer>(
				cmd, "/lenMax", new IntegerValue(), 25);

		SingleArgumentOption<String> infile =
			new SingleArgumentOption<String>(
				cmd, "/infile", new StringValue(), null);
		SingleArgumentOption<String> outfile =
			new SingleArgumentOption<String>(
				cmd, "/outfile", new StringValue(), null);
		SingleArgumentOption<String> gcfile =
			new SingleArgumentOption<String>(
				cmd, "/gcfile", new StringValue(), null);
		SingleArgumentOption<String> profile =
			new SingleArgumentOption<String>(
				cmd, "/profile", new StringValue(), null);
		SingleArgumentOption<Integer> quantity =
			new SingleArgumentOption<Integer>(
				cmd, "/q", new IntegerValue(), 20);
		SingleArgumentOption<Integer> start =
			new SingleArgumentOption<Integer>(
				cmd, "/startingpoint", new IntegerValue(), 1);
		SingleArgumentOption<Integer> end =
			new SingleArgumentOption<Integer>(
				cmd, "/endpoint", new IntegerValue(), -1);

		SingleArgumentOption<Float> nx =
			new SingleArgumentOption<Float>(
				cmd, "/nx", new FloatValue(), (float)1);
		SingleArgumentOption<Float> ny =
			new SingleArgumentOption<Float>(
				cmd, "/ny", new FloatValue(), (float)1);
		SingleArgumentOption<Float> pa =
			new SingleArgumentOption<Float>(
		...
		NoArgumentOption complementary = new NoArgumentOption(
		...
		SingleArgumentOption<Float[]> tmOpt =
			new SingleArgumentOption<Float[]>(
				cmd, "/tm", new FloatArrayValue(), new Float[]{50f,65f});
		NoArgumentOption notmOpt = new NoArgumentOption(cmd, "/notm");

		SingleArgumentOption<End5v3Value.Result> end5v3 =
			new SingleArgumentOption<End5v3Value.Result>(
				cmd,
				"/end5v3",
				new End5v3Value(),
		...
		SingleArgumentOption<Integer> baserun =
			new SingleArgumentOption<Integer>(
				cmd, "/baserun", new IntegerValue(), 4);
		NoArgumentOption nobaserun = new NoArgumentOption(cmd, "/nobaserun");

		SingleArgumentOption<Integer> homodimer =
			new SingleArgumentOption<Integer>(
				cmd, "/homodimer", new IntegerValue(), 5);
		NoArgumentOption nohomodimer = new NoArgumentOption(cmd, "/nohomodimer");

		SingleArgumentOption<Integer> homodimerfixedEnd =
			new SingleArgumentOption<Integer>(
				cmd, "/homodimer3", new IntegerValue(), 3);
		NoArgumentOption nohomodimerfixedEnd =
			new NoArgumentOption(cmd, "/nohomodimer3");

		SingleArgumentOption<Float[]> gccontent =
			new SingleArgumentOption<Float[]>(
				cmd, "/gc", new FloatArrayValue(), new Float[]{40f,60f});
		NoArgumentOption nogccontent = new NoArgumentOption(cmd, "/nogccontent");

		SingleArgumentOption<Float> score = new SingleArgumentOption<Float>(
			cmd, "/score", new FloatValue(), 0.8f);
		NoArgumentOption noscore = new NoArgumentOption(cmd, "/noscore");

		SingleArgumentOption<Integer> ampsize =
			new SingleArgumentOption<Integer>(
				cmd, "/size", new IntegerValue(), 200);
		NoArgumentOption noampsize = new NoArgumentOption(cmd, "/nosize");

		SingleArgumentOption<Integer> smallampsize =
			new SingleArgumentOption<Integer>(
				cmd, "/minsize", new IntegerValue(), 100);
		NoArgumentOption nosmallampsize =
			new NoArgumentOption(cmd, "/nominsize");

		SingleArgumentOption<Float> gccomp =
			new SingleArgumentOption<Float>(
				cmd, "/gccomp", new FloatValue(), 10f);
		NoArgumentOption nogccomp = new NoArgumentOption(cmd, "/nogccomp");

		SingleArgumentOption<Integer> heterodimer =
			new SingleArgumentOption<Integer>(
				cmd, "/hetdimer", new IntegerValue(), 5);
		NoArgumentOption noheterodimer =
			new NoArgumentOption(cmd, "/nohetdimer");

		SingleArgumentOption<Integer> heterodimerfixedEnd =
			new SingleArgumentOption<Integer>(
				cmd, "/hetdimer3", new IntegerValue(), 3);
		NoArgumentOption noheterodimerfixedEnd =
			new NoArgumentOption(cmd, "/nohetdimer3");

		SingleArgumentOption<Float> tmcomp =
			new SingleArgumentOption<Float>(
		...
		try {
			...
			System.err.println(
				"There was an error trying to parse the command line:"
			);
		...
		if (isDna.isPresent()&&isProtein.isPresent()) {
			...
			throw new InvalidCommandLineException(
				"isDna option and isProtein Option can not be in the command line at the same time."
			);
		}
		...
		if (tmOpt.isPresent() && notmOpt.isPresent() ) {
			...
			throw new InvalidCommandLineException(
				"/tm and /notm options can not appear in the command line simoultaneously"
			);
		}
		if (end5v3.isPresent() && noend5v3.isPresent() ) {
 			// This options can't be in the command line at the same time
			throw new InvalidCommandLineException(
				"/end5v3 and /noend5v3 options can not appear in the command line simoultaneously"
			);
		}
		if (baserun.isPresent() && nobaserun.isPresent() ) {
 			// This options can't be in the command line at the same time
			throw new InvalidCommandLineException(
				"/baserun and /nobaserun options can not appear in the command line simoultaneously"
			);
		}
		if (homodimer.isPresent() && nohomodimer.isPresent() ) {
 			// This options can't be in the command line at the same time
			throw new InvalidCommandLineException(
				"/homodimer and /nohomodimer options can not appear in the command line simoultaneously"
			);
		}
		...
		if (!pair.isPresent()) {
			...
			if (isAnyPrimerPairParameter) {
				...
				throw new InvalidCommandLineException(
					"/pair option is not present in the command line, but there is one or more parameters for primer pair search."
				);
			}
		}
		...
		TmEstimator tme;
		if (this.isUseSantaLuciaToEstimateTm()) {
		...
		if (filterRep.getValue()) vffsp.add(
			new ValidateForFilterSinglePrimer(
				new FilterRepeatedEnd()));
		if (filterDeg.getValue()) vffsp.add(
			new ValidateForFilterSinglePrimer(
				new FilterDegeneratedEnd()));
		if (! noscore.getValue()) vffsp.add(
			new ValidateForFilterSinglePrimer(
				new FilterPrimerScore(score.getValue())));
		if (! notmOpt.isPresent()) vffsp.add(
			new ValidateForFilterSinglePrimer(
				new FilterMeltingPointTemperature(
					((Float[])tmOpt.getValue())[0],
					((Float[])tmOpt.getValue())[1],
					tme)
				));
		End5v3Value.Result r = end5v3.getValue();
		if (! noend5v3.isPresent()) vffsp.add(
			new ValidateForFilterSinglePrimer(
				new Filter5vs3Stability(
					r.dg, r.ktemp, r.monov, r.len)));
		if (! nobaserun.isPresent()) vffsp.add(
			new ValidateForFilterSinglePrimer(
				new FilterBaseRuns((Integer) baserun.getValue())));
		if (! nohomodimer.isPresent()) vffsp.add(
			new ValidateForFilterSinglePrimer(
				new FilterHomoDimer(
					(Integer) homodimer.getValue(),
					new DegeneratedDNAMatchingStrategy())));
		if (! nohomodimerfixedEnd.isPresent()) vffsp.add(
			new ValidateForFilterSinglePrimer(
				new FilterHomoDimerFixed3(
					(Integer) homodimerfixedEnd.getValue(),
					new DegeneratedDNAMatchingStrategy())));
		if (! nogccontent.isPresent()) vffsp.add(
			new ValidateForFilterSinglePrimer(
				new FilterCGContent(
					((Float[]) gccontent.getValue())[0],
					((Float[]) gccontent.getValue())[1])));
		...
		if ( pair.isPresent()) {
			...
			if (! noampsize.isPresent()) vffpp.add(
				new ValidateForFilterPrimerPair(
					new FilterAmpliconSize((Integer) ampsize.getValue())));
			if (! nosmallampsize.isPresent()) vffpp.add(
				new ValidateForFilterPrimerPair(
					new FilterSmallAmpliconSize((Integer) smallampsize.getValue())));
			if (! nogccomp.isPresent()) vffpp.add(
				new ValidateForFilterPrimerPair(
					new FilterGCCompatibility((Float) gccomp.getValue())));
			if (! noheterodimer.isPresent()) vffpp.add(
				new ValidateForFilterPrimerPair(
					new FilterHeteroDimer(
						(Integer) heterodimer.getValue(),
						new DegeneratedDNAMatchingStrategy())));
			if (! noheterodimerfixedEnd.isPresent()) vffpp.add(
				new ValidateForFilterPrimerPair(
					new FilterHeteroDimerFixed3(
						(Integer) heterodimerfixedEnd.getValue(),
						new DegeneratedDNAMatchingStrategy())));
			if (! notmcomp.isPresent()) vffpp.add(
				new ValidateForFilterPrimerPair(
